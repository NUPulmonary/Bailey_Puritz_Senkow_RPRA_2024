...
## Renormalize abundances (presumably all rounding errors)
...
pheatmap2ggplot <- function (x) {
    patchwork::wrap_elements(grid::grid.grabExpr(ComplexHeatmap::draw(x)))
}
...
heatmap <- ComplexHeatmap::pheatmap(fmat_scaled,
                                    cluster_rows = T,
                                    cluster_cols = T,
                                    show_colnames = T,
                                    scale = "none",
...
ct_data <- read.csv("deidentified_data/deidentified_ct_scan_1_data.csv", check.names = FALSE)
...
cor_df_full <- dplyr::full_join(cor_df, p_df, by = c("Var1", "Var2")) %>%
    dplyr::mutate(padj = p_adj)
write.csv(cor_df_full, file = "flow/ct_correlation.csv")
...
ct_corr <- ComplexHeatmap::pheatmap(cor_mat,
                                    cluster_rows = T,
                                    cluster_cols = T,
                                    cellwidth = 20,
                                    cellheight = 20,
    ...
    for (j in 1:length(ct_features)) {
        ...
        pplt <- ggplot(df, aes(x, y)) +
            ...
            annotate("text", x = 0.73 * max(df$x), y = 0.98 * max(df$y), hjust = 0,
                     label = paste0("rho == ", deparse(sprintf("%.3f", cor(df$x, df$y, method = "s")))),
                     parse = TRUE) +
            theme_bw() +
            theme(text = element_text(family = "Arial"),
...
fig_2a <- patchwork::wrap_elements(heatmap + inset_element(cl_bar, 0.025, -0.16, 0.809, 0.9) + get_annot("a"))
...
## Build Figure S2
fnames <- c("Neutrophils_Normal", "Neutrophils_Inflammatory", "Neutrophils_Nodularity",
            "CD4_T_cells_Fibrotic", "B_and_plasma_cells_Fibrotic", "CD206_high_MPs_Nodularity")
ylabel <- ggplot(data.frame(l = "Percent of total cells", x = 1, y = 1)) +
    geom_text(aes(x, y, label = l), angle = 90, size = 4) +
    theme_void() +
    coord_cartesian(clip = "off")
