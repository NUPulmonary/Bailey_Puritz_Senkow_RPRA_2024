...
deg_dir <- "/projects/b1038/Pulmonary/cpuritz/PASC/data/01NEP/DEG"
count_dir <- sprintf("%s/counts", deg_dir)
out_dir <- sprintf("%s/output", deg_dir)
cell_names <- read.csv(sprintf("%s/cell_names.csv", count_dir),
                       check.names = FALSE)
# Too few fibroblasts, no samples were retained
cell_names <- cell_names[cell_names$type != "Fibroblasts",]
